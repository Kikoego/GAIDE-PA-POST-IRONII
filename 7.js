...
function load_student(id) {
    ...
    span = document.getElementById('scores')
    span.textContent = " " + students[id].scores
    for (i = 0; i <= students.length; i++) {
        let sum = 0 + students[i]
        let mid = sum / students.length
        let span = document.getElementById('midMark')
        span.textContent = " " + mid.midMark
    }
}
function next() {
    id_current++
